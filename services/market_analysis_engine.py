...
# Regime accuracy tracking (see core/application/interfaces/regime_call_repository.py):
# 30 min is the headline win-rate horizon shown in the Decision panel — 15m
# and 60m are still logged and stored, just not surfaced there. Below
# _ACCURACY_MIN_SAMPLE, the stat block shows "not enough data yet" instead
# of a win rate that'd be noise at low n.
_ACCURACY_HORIZON = "30m"
_ACCURACY_HORIZON_MINUTES = 30
_ACCURACY_LOOKBACK_DAYS = 30
_ACCURACY_MIN_SAMPLE = 10
...
class MarketAnalysisEngine:
    def __init__(self, zerodha_client, regime_call_repository: Optional[IRegimeCallRepository] = None):
        self.zerodha_client = zerodha_client
        # Optional so existing callers/tests that construct this with just
        # a client keep working unchanged — accuracy tracking is additive.
        self.regime_call_repository = regime_call_repository

    def _log_and_score_accuracy(self, symbol: str, logged_at, ltp: float, classification: dict) -> Optional[dict]:
        """Logs this call for later win-rate scoring, then returns the
        rolling accuracy stat for this exact (symbol, decision) pair — or
        None if there's no repository wired up or no side was suggested
        (a NO TRADE call has no win/loss to track)."""
        if self.regime_call_repository is None or logged_at is None:
            return None

        try:
            self.regime_call_repository.log_call(RegimeCall(
                symbol=symbol,
                logged_at=logged_at,
                ltp=ltp,
                ...
                suggested_side=classification["suggested_side"],
            ))
        except Exception:
            logger.exception("Failed to log regime call for %s", symbol)
            return None

        if classification["suggested_side"] is None:
            return None

        try:
            since = dt.datetime.now() - dt.timedelta(days=_ACCURACY_LOOKBACK_DAYS)
            stat = self.regime_call_repository.win_rate(
                symbol, classification["decision"], _ACCURACY_HORIZON, since
            )
        except Exception:
            logger.exception("Failed to compute regime call accuracy for %s", symbol)
            return None

        return {
            "win_rate": round(stat.win_rate, 1) if stat.n >= _ACCURACY_MIN_SAMPLE else None,
            "n": stat.n,
            "min_sample": _ACCURACY_MIN_SAMPLE,
            "horizon_minutes": _ACCURACY_HORIZON_MINUTES,
            "lookback_days": _ACCURACY_LOOKBACK_DAYS,
        }
    ...
    def analyze(self, symbol: str) -> dict:
        ...
        # Naive wall-clock IST, matching how the rest of the app stores
        # timestamps (e.g. TradeRecord.closed_at) — this is what
