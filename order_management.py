...
def execute_buy(entry_price):
    """
    Execute a buy order and initialize trade state.
    """
...
def execute_exit(reason, price):
    ...
    trade_time = pd.Timestamp.now()  # or tick['timestamp'] if available
    update_pnl(pnl, trade_time=trade_time, entry_price=entry_price, latest_vwap=None)  # VWAP optional here
...
def check_exit_conditions(tick, data):
    """
    Check stop-loss, target, trailing SL, and time-based exit.
    ...
    # Trailing Stop-loss check (VWAP-based)
