...
class IndicatorCalculator:
    ...
    def rsi(symbol, df, period=14):
        ...
        if len(rsi) > 1:  # ensure enough values
            RSI[symbol] = rsi.iloc[-2]  # return previous RSI
        return RSI
...
class IndicatorCalculator:
    ...
    def macd(symbol, df, fastperiod=12, slowperiod=26, signalperiod=9):
        ...
        if len(macd) > 1 and len(macdsignal) > 1 and len(macdhist) > 1:
            MACD[symbol] = {
                "macd": macd.iloc[-2],
                "signal": macdsignal.iloc[-2],
                "hist": macdhist.iloc[-2],
            }
        return MACD
    ...
    def vwap(symbol, df):
        ...
        if cum_vol.iloc[-2] > 0:
            VWAP[symbol] = float(cum_pv.iloc[-2] / cum_vol.iloc[-2])
        return VWAP
